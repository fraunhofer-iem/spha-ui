...
<template>
  <div class="mt-5">
    <div
        :class="['p-5 rounded-3 bg-light border border-3 border-dashed text-center', { 'border-primary bg-primary-subtle shadow-sm': isDragging }]"
    ...
    >
      ...
      <div class="text-secondary">
        ...
        <p v-if="!fileName" class="mb-0">Drag & drop a SPHA result file here, or click to select.</p>
        <p v-if="fileName" class="fw-bold text-dark mb-0">File: {{ fileName }}</p>
        <p v-if="error" class="text-danger fw-bold mt-2 mb-0">{{ error }}</p>
        <p v-if="successMessage" class="text-success fw-bold mt-2 mb-0">{{ successMessage }}</p>
      </div>
    </div>
  </div>
</template>
